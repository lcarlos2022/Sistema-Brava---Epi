...
class SistemaEPIApp:
  ...
  def __init__(self, root):
    ...
    self.style.map(
        "TNotebook.Tab",
        background=[("selected", "#009b3a")],
        foreground=[("selected", "#ffdf00")],
    )
  ...
  def criar_tela_login(self):
    ...
    tk.Label(
        self.login_frame,
        text="Controle de Estoque e EPIs",
        font=("Arial", 11),
        fg="#cccccc",
        bg="#141414",
    ).pack(pady=(0, 20))
    ...
    self.entry_senha = ttk.Entry(
        frame_senha, width=25, show="*", font=("Arial", 11)
    )
    ...
    btn_cad_tst = tk.Button(
        self.login_frame,
        text="Cadastrar Novo TST",
        command=self.abrir_janela_cadastro_tst,
        font=("Arial", 9, "bold"),
        bg="#222222",
        fg="#ffdf00",
  ...
  def abrir_janela_cadastro_tst(self):
    ...
    def salvar_novo_tst():
      ...
      if any(u["usuario"].upper() == nome for u in usuarios):
        messagebox.showerror(
            "Erro", "Este usuário já está cadastrado!", parent=janela_tst
        )
        return

      usuarios.append(
          {"usuario": nome, "senha": "12345678", "mudar_senha": True}
      )
      salvar_json(ARQUIVO_USUARIOS, usuarios)
      messagebox.showinfo(
          "Sucesso",
          f"TST '{nome}' cadastrado!\nSenha padrão inicial:"
    ...
    btn_salvar = tk.Button(
        janela_tst,
        text="Cadastrar TST (Senha Padrão 12345678)",
        command=salvar_novo_tst,
        font=("Arial", 9, "bold"),
        bg="#009b3a",
        fg="#ffffff",
        relief="flat",
  ...
  def abrir_janela_alterar_senha(self, usuario_nome):
    ...
    tk.Label(
        janela_alt,
        text="Por favor, cadastre uma nova senha forte.",
        font=("Arial", 9),
        fg="#cccccc",
        bg="#141414",
    ).pack(pady=(0, 10))
    ...
    tk.Label(
        janela_alt,
        text="Nova Senha (8+ chars, Maiúscula, Especial, etc):",
        font=("Arial", 9, "bold"),
        fg="#009b3a",
        bg="#141414",
    ).pack(anchor="w", padx=30)
    ...
    def salvar_nova_senha():
      ...
      messagebox.showinfo(
          "Sucesso",
          "Senha alterada com sucesso! Entrando no sistema...",
          parent=janela_alt,
      )
      janela_alt.destroy()
    ...
    btn_alt = tk.Button(
        janela_alt,
        text="Salvar Nova Senha e Entrar",
  ...
  def verificar_login(self):
    ...
    tst_encontrado = next(
        (
            ...
            if u["usuario"].upper() == usuario.upper() and u["senha"] == senha
        ),
        None,
    )
  ...
  def iniciar_sistema_principal(self):
    ...
    tk.Label(
        topo_frame,
        text=f"👤 Logado como: {self.usuario_atual}",
        font=("Arial", 10, "bold"),
        bg="#141414",
        fg="#ffdf00",
  ...
  def construir_aba_produtos(self):
    tk.Label(
        self.frame_produtos,
        text="Gerenciamento de Códigos, Tamanhos e Estoque",
        font=("Arial", 11, "bold"),
        bg="#0a0a0a",
        fg="#009b3a",
    ).pack(pady=10)
    ...
    tk.Button(
        f1,
        text="Cadastrar",
        command=self.salvar_novo_produto,
        bg="#009b3a",
        fg="#ffffff",
        font=("Arial", 9, "bold"),
        relief="flat",
    ...
    self.tree_prod = ttk.Treeview(
        f2,
        columns=("codigo", "descricao", "tamanho", "ca", "estoque"),
        show="headings",
        height=12,
    )
    for col, txt, larg, alinh in [
        ("codigo", "Código", 90, "center"),
        ("descricao", "Descrição do EPI", 340, "w"),
        ("tamanho", "Tamanho", 80, "center"),
        ("ca", "CA", 90, "center"),
        ("estoque", "Qtd Estoque", 90, "center"),
    ]:
    ...
    tk.Button(
        f3,
        text="💾 Salvar",
        command=self.atualizar_produto_selecionado,
        bg="#009b3a",
        fg="#ffffff",
        font=("Arial", 9, "bold"),
        relief="flat",
  ...
  def salvar_novo_produto(self):
    cod, desc, tam, ca, qtd_est = (
        self.e_cod.get().strip(),
        self.e_desc.get().strip(),
        self.e_tam.get().strip(),
        self.e_ca.get().strip(),
        self.e_qtd_estoque.get().strip(),
    )
  ...
  def construir_aba_colaboradores(self):
    ...
    tk.Label(
        topo_colab,
        text="Gerenciamento e Cadastro de Colaboradores",
        font=("Arial", 11, "bold"),
        bg="#0a0a0a",
        fg="#009b3a",
    ).pack(side="left")
    self.lbl_total_obra = tk.Label(
        topo_colab,
        text="👷 Total na Obra: 0",
        font=("Arial", 11, "bold"),
        bg="#0a0a0a",
        fg="#ffdf00",
    )
    ...
    tk.Label(f1, text="Matrícula:", bg="#141414", fg="#ffffff").grid(
        row=0, column=0, sticky="w", padx=3
    )
    self.e_mat = ttk.Entry(f1, width=12)
    self.e_mat.grid(row=0, column=1, padx=3, pady=5)

    tk.Label(f1, text="Nome Completo:", bg="#141414", fg="#ffffff").grid(
        row=0, column=2, sticky="w", padx=3
    )
    self.e_nome = ttk.Entry(f1, width=32)
    self.e_nome.grid(row=0, column=3, padx=3, pady=5)

    tk.Label(f1, text="Função:", bg="#141414", fg="#ffffff").grid(
        row=0, column=4, sticky="w", padx=3
    )
    ...
    f2 = self.criar_bloco(
        self.frame_colab,
        "Lista de Colaboradores Cadastrados (Clique para selecionar e editar)",
    )
    f2.pack(fill="both", expand=True, padx=15, pady=10, ipadx=5, ipady=5)

    self.tree_colab = ttk.Treeview(
        f2,
        columns=("matricula", "nome", "funcao"),
        show="headings",
        height=12,
    )
    for col, txt, larg, alinh in [
        ("matricula", "Matrícula", 110, "center"),
        ("nome", "Nome Completo", 420, "w"),
        ("funcao", "Função", 260, "w"),
    ]:
    ...
    self.tree_colab.bind(
        "<<TreeviewSelect>>", self.carregar_colaborador_selecionado
    )

    f3 = self.criar_bloco(
        self.frame_colab, "Alterar / Editar ou Excluir Colaborador Selecionado"
    )
    f3.pack(fill="x", padx=15, pady=5, ipadx=5, ipady=5)

    tk.Label(f3, text="Matrícula:", bg="#141414", fg="#ffffff").grid(
        row=0, column=0, padx=2
    )
    self.e_edit_mat = ttk.Entry(f3, width=12)
    self.e_edit_mat.grid(row=0, column=1, padx=2)

    tk.Label(f3, text="Nome:", bg="#141414", fg="#ffffff").grid(
        row=0, column=2, padx=2
    )
    self.e_edit_nome = ttk.Entry(f3, width=32)
    self.e_edit_nome.grid(row=0, column=3, padx=2)

    tk.Label(f3, text="Função:", bg="#141414", fg="#ffffff").grid(
        row=0, column=4, padx=2
    )
    ...
    tk.Button(
        f3,
        text="💾 Salvar",
        command=self.atualizar_colaborador_selecionado,
        bg="#009b3a",
        fg="#ffffff",
        font=("Arial", 9, "bold"),
        relief="flat",
    ...
    tk.Button(
        f3,
        text="🗑️ Excluir",
        command=self.excluir_colaborador_selecionado,
        bg="#cc0000",
        fg="#ffffff",
        font=("Arial", 9, "bold"),
        relief="flat",
  ...
  def atualizar_tabela_colaboradores(self):
    ...
    for c in colaboradores:
      self.tree_colab.insert(
          "", "end", values=(c["matricula"], c["nome"], c["funcao"])
      )
    if hasattr(self, "lbl_total_obra"):
      self.lbl_total_obra.config(
          text=f"👷 Total na Obra: {len(colaboradores)}"
      )
  ...
  def excluir_colaborador_selecionado(self):
    ...
    if messagebox.askyesno("Confirmação", f"Remover {v[1]}?"):
      colaboradores = carregar_json(
          ARQUIVO_COLABORADORES, COLABORADORES_INICIAIS
      )
      colaboradores = [
          c
          for c in colaboradores
          if str(c["matricula"]).strip() != str(v[0]).strip()
      ]
      salvar_json(ARQUIVO_COLABORADORES, colaboradores)
      self.atualizar_tabela_colaboradores()
  ...
  def construir_aba_entrega(self):
    tk.Label(
        self.frame_entrega,
        text="Registro de Entrega de EPI",
        font=("Arial", 11, "bold"),
        bg="#0a0a0a",
        fg="#009b3a",
    ).pack(pady=10)
    ...
    tk.Label(
        f1, text="Matrícula do Colaborador:", bg="#141414", fg="#ffffff"
    ).grid(row=0, column=0, sticky="w", padx=5, pady=5)
    self.var_matricula = tk.StringVar()
    self.var_matricula.trace_add(
        "write", lambda *args: self.buscar_colaborador_automatico()
    )
    ttk.Entry(f1, textvariable=self.var_matricula, width=20).grid(
        row=0, column=1, sticky="w", padx=5, pady=5
    )

    self.lbl_info_colab = tk.Label(
        f1,
        text="Colaborador: [Aguardando...]",
        font=("Arial", 10, "bold"),
        bg="#141414",
        fg="#ffdf00",
    )
    self.lbl_info_colab.grid(
        row=1, column=0, columnspan=3, sticky="w", padx=5, pady=5
    )

    tk.Label(f1, text="Produto (EPI):", bg="#141414", fg="#ffffff").grid(
        row=2, column=0, sticky="w", padx=5, pady=5
    )
    self.combo_prod = ttk.Combobox(f1, width=65, state="readonly")
    self.combo_prod.grid(
    ...
    tk.Label(f1, text="Quantidade Entregue:", bg="#141414", fg="#ffffff").grid(
    ...
    tk.Button(
        ...
        padx=10,
        pady=5,
    ).grid(row=4, column=1, pady=15, sticky="w")

  def buscar_colaborador_automatico(self):
    mat = self.var_matricula.get().strip()
    if not mat:
      self.lbl_info_colab.config(
          text="Colaborador: [Aguardando...]", fg="#ffdf00"
      )
      self.colab_encontrado = None
      return
    colaboradores = carregar_json(ARQUIVO_COLABORADORES, COLABORADORES_INICIAIS)
    match = next(
        (c for c in colaboradores if str(c["matricula"]).strip() == mat), None
    )
    if match:
      self.colab_encontrado = match
      self.lbl_info_colab.config(
          text=f"✅ {match['nome']} - Função: {match['funcao']}", fg="#009b3a"
      )
    else:
      self.colab_encontrado = None
      self.lbl_info_colab.config(
          text="⚠️ Matrícula não encontrada", fg="#ff4444"
      )
  ...
  def salvar_entrega(self):
    ...
    match_cod = re.search(r"Cód:\s*(.*?)\s*-", selecao_prod)
    if not match_cod:
      messagebox.showerror("Erro", "Erro ao identificar o código do produto!")
      return
    cod_epi = match_cod.group(1).strip()

    produtos = carregar_json(ARQUIVO_PRODUTOS, [])
    prod_obj = next(
        (p for p in produtos if str(p["codigo"]).strip() == cod_epi), None
    )
    ...
    estoque_atual = int(prod_obj.get("estoque", 0))
    if estoque_atual < qtd:
      messagebox.showerror(
          "Erro Estoque Insuficiente",
          f"Estoque atual ({estoque_atual}) é menor do que a quantidade solicitada"
          f" ({qtd})!",
      )
      return

    prod_obj["estoque"] = estoque_atual - qtd
    salvar_json(ARQUIVO_PRODUTOS, produtos)
    ...
    entregas.append({
        ...
        "colaborador": self.colab_encontrado["nome"],
        "funcao": self.colab_encontrado["funcao"],
        "produto": selecao_prod,
        "quantidade": qtd,
    ...
    messagebox.showinfo(
        "Sucesso",
        f"Entrega de {qtd} un. registrada para"
        f" {self.colab_encontrado['nome']}!\nEstoque atualizado.",
    )
  ...
  def construir_aba_historico(self):
    tk.Label(
        self.frame_hist,
        text="📜 Histórico Geral de Entregas de EPIs",
        font=("Arial", 11, "bold"),
        bg="#0a0a0a",
        fg="#009b3a",
    ).pack(pady=10)

    f1 = self.criar_bloco(self.frame_hist, "Registros de Entregas Anteriores")
    f1.pack(fill="both", expand=True, padx=15, pady=10, ipadx=5, ipady=5)
    ...
    for col, txt, larg, alinh in [
        ...
        ("matricula", "Matrícula", 90, "center"),
        ("colaborador", "Colaborador", 300, "w"),
        ("produto", "EPI Entregue", 340, "w"),
        ("qtd", "Qtd", 60, "center"),
        ("responsavel", "TST Responsável", 160, "w"),
    ]:
    ...
    btn_atualizar_hist.pack(pady=5)
